...
@section('page-title') Shopalldeals - {{$cat->name}} @endsection

@section('meta-description'){{'Find best '. $cat->name . ' deals and coupons in the UEA - Shopalldeals'}}@endsection
...
<div class="row mb-2">
    <div class="col-sm-12">
        ...
        <span> <a href="{{route('home')}}">Deals</a> 
         ...
         @if(isset( $subcat ))
          /
          <a href="{{route('front.category', ['catSlug' => $cat->slug, 'subCatSlug' => $subcat->slug])}}">{{$subcat->name}}</a> 
          @endif
          @if(isset( $childcat ))
          /
          <a href="{{route('front.category', ['catSlug' => $cat->slug, 'subCatSlug' => $subcat->slug, 'childCatSlug' => $childcat->slug,])}}">{{$childcat->name}}</a> 
          @endif
        </span>
      </div>
</div>
  ...
  function loadMore() {
    ...
    if ( isNoMoreDeals ) {
      ...
      if (context == 'filter') {
        ...
        var url = '/categories/' + next_page + '/' + '{{$cat->slug ?? ""}}' + '/' + '{{$subcat->slug ?? ""}}' + '/' + '{{$childcat->slug ?? ""}}';
      }
